...
def gen_frames():
    cred = credentials.Certificate(r"Face_Attedance\serviceAccountKey.json")
    firebase_admin.initialize_app(
        cred,
        {
            "databaseURL": "https://face-attedance-22cd8-default-rtdb.firebaseio.com/",
            "storageBucket": "face-attedance-22cd8.appspot.com",
        },
    )
    ...
    imgBackground = cv2.imread(
        r"Face_Attedance\Resources\background.png"
    )

    # Importing the mode images into a list
    folderModePath = r"Face_Attedance\Resources\Modes"
    ...
    file = open(r"Face_Attedance\EncodeFile.p", "rb")
            ...
            if counter != 0:
                if counter == 1:
                    ...
                    if studentInfo is not None:
                        ...
                        blob = bucket.get_blob(f"Files/Images/{id}.jpg")
                        array = np.frombuffer(blob.download_as_string(), np.uint8)
                        imgStudent = cv2.imdecode(array, cv2.IMREAD_COLOR)
                        imgStudent = cv2.resize(
                            imgStudent, (216, 216)
                        )  # Resize imgStudent to match imgBackground slice
                        ...
                        print("Student info not found for id:", id)
